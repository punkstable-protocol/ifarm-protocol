...
contract IFAPool is Ownable {
    ...
    function setPoolInfo(uint256 _poolId, IERC20 _token, IFAVault _vault, uint256 _startTime) public onlyOwner {
        poolMap[_poolId].token = _token;
        poolMap[_poolId].vault = _vault;
        poolMap[_poolId].startTime = _startTime;
    }

    function _handleDeposit(IFAVault _vault, IERC20 _token, uint256 _amount) internal {
    ...
    function _handleWithdraw(IFAVault _vault, IERC20 _token, uint256 _amount) internal {
        uint256 count = _vault.getStrategyCount();
    ...
    function _handleRewards(IFAVault _vault) internal {
        uint256 count = _vault.getStrategyCount();
    ...
    // Deposit tokens to IFAPool for IFA allocation.
    ...
    // Claim IFA (and potentially other tokens depends on strategy).
    ...
    // Withdraw tokens from IFAPool (from a strategy first if there is one).

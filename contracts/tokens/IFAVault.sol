...
// IFAVault is owned by Timelock
contract IFAVault is ERC20, Ownable {
    ...
    constructor (IFAMaster _ifaMaster, string memory _name, string memory _symbol) ERC20(_name, _symbol) public  {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by IFAPool.
    function mintByPool(address _to, uint256 _amount) public {
        require(_msgSender() == ifaMaster.pool(), "not pool");
    ...
    // Must only be called by IFAPool.
    ...
    // Must only be called by IFABank.
    function transferByBank(address _from, address _to, uint256 _amount) public {
        require(_msgSender() == ifaMaster.bank(), "not bank");
